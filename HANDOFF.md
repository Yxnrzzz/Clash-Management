...
|---|---|---|---|
| 0 | Fondasi backend | ✅ Selesai | NestJS + Prisma + PostgreSQL, 4 migrasi ter-apply, seed idempoten, `/api/health`. **CI/Dockerfile masih belum ada** (lihat §12) |
...
**`npm --prefix apps/api run lint` tidak dijalankan di sini secara sengaja** — `apps/api` tidak pernah punya `eslint.config.mjs` sendiri sejak awal proyek, jadi perintah itu gagal total ("all files ignored"), bukan cuma warning. Lihat §12 kalau mau memperbaikinya (kandidat Sprint 11).
...
- **Dockerfile + CI** — belum ada sama sekali. Deliverable CI Sprint 0 sebenarnya belum terpenuhi. **Rekomendasi urutan berikutnya**: kerjakan ini dulu (murah, cepat, menutup utang Sprint 0) sebelum masuk Sprint 11 penuh.
- **Lint backend tidak pernah benar-benar jalan** — `npm --prefix apps/api run lint` gagal dari awal proyek (`eslint.config.mjs` cuma ada di root, untuk Next.js; `apps/api` tidak pernah punya config eslint sendiri meski `package.json`-nya mendefinisikan script `lint`). Bukan regresi dari update ke-9 — ditemukan saat verifikasi sesi ini. Baru ketahuan sekarang karena checklist §10 sebelumnya cuma menyebut `npm run lint` (frontend) tanpa versi `--prefix apps/api`. Perlu `eslint.config.mjs` baru di `apps/api` sebelum lint backend bisa dipakai — kandidat kerjaan Sprint 11/hardening.
- **Worktree nyasar** — `D:\WebApp\.claude\worktrees\fervent-gauss-19c30b` (checkout terpisah, HEAD di commit awal `66d0c53`) bikin `npm run lint` di root melaporkan ribuan error palsu (linting `.next`/`node_modules` hasil build worktree itu sendiri) karena `eslint.config.mjs` di root tidak meng-exclude `.claude/`. Bukan dibuat sesi ini — ditemukan saat verifikasi. Kalau worktree itu memang sudah tidak dipakai, hapus dengan `git worktree remove` (bukan `rm -rf` manual); kalau tidak yakin, tanya user dulu sebelum menghapus.
